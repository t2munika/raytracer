...
extern "C"
int gr_union_csg_cmd(lua_State* L) {
  GRLUA_DEBUG_CALL;
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new CSGNode(name, CSGNode::UNION);
...
//create a CSG intersect node
extern "C"
int gr_intersect_csg_cmd(lua_State* L) {
  GRLUA_DEBUG_CALL;
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new CSGNode(name, CSGNode::INTERSECTION);
...
int gr_difference_csg_cmd(lua_State* L) {
  GRLUA_DEBUG_CALL;
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new CSGNode(name, CSGNode::DIFFERENCE);
...
int gr_plane_cmd(lua_State* L)
{
  ...
  const char* name = luaL_checkstring(L, 1);
  std::string type = std::string(luaL_checkstring(L, 2));
  ...
  if (type == "xy") {
	Plane* plane = new Plane(Plane::XY);
	GRLUA_DEBUG(*plane);
	data->node = new GeometryNode(name, plane);
  } else if (type == "xz") {
	Plane* plane = new Plane(Plane::XZ);
	GRLUA_DEBUG(*plane);
	data->node = new GeometryNode(name, plane);
  } else if (type == "yz") {
	Plane* plane = new Plane(Plane::YZ);
	GRLUA_DEBUG(*plane);
	data->node = new GeometryNode(name, plane);
  } else {
    luaL_argcheck(L, false, 2, "invalid plane type");	
  }
...
int gr_render_cmd(lua_State* L)
{
  ...
  double focal_length = view.length();
  double aperture = 0;
  if (lua_gettop(L) == 12) {
	focal_length = luaL_checknumber(L, 11); 
	aperture = luaL_checknumber(L, 12);
  }
  ...
  a4_render(root->node, filename, width, height,
			...
			focal_length, aperture, settings);
...
int gr_material_cmd(lua_State* L)
{
  ...
  double refract_index = 1.62;
  if (lua_gettop(L) == 6) {
	transparency = luaL_checknumber(L, 4);
	refract_index = luaL_checknumber(L, 5);
  }
  
  data->material = new PhongMaterial(Colour(kd[0], kd[1], kd[2]),
                                     Colour(ks[0], ks[1], ks[2]),
                                     shininess, transparency, refract_index);
...
int gr_material_set_normal_map_cmd(lua_State* L) {
  GRLUA_DEBUG_CALL;
...
extern "C"
int gr_material_gc_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  
  gr_material_ud* data = (gr_material_ud*)luaL_checkudata(L, 1, "gr.node");
...
static const luaL_reg grlib_functions[] = {
  ...
  {"union", gr_union_csg_cmd},
  {"intersect", gr_intersect_csg_cmd},
  {"difference", gr_difference_csg_cmd},
...
static const luaL_reg grlib_material_methods[] = {
  {"__gc", gr_material_gc_cmd},
  {"set_normal_map", gr_material_set_normal_map_cmd},
...
bool run_lua(const std::string& filename, Settings s)
{
  ...
  // Set up the metatable for gr.materials
  luaL_newmetatable(L, "gr.material");
  lua_pushstring(L, "__index");
  lua_pushvalue(L, -2);
  lua_settable(L, -3);
